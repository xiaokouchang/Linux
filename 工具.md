...
## 理 解
程 序 还 是 代 码 都 是 数 据。
一 个 操 作 系 统 不 仅 只 能 运 行 1 个 进 程，还 可 以 同 时 运 行 多 个 进 程。
操 作 系 统 必 须 将 进 程 管 理 起 来。先 描 述，再 组 织。
任 何 一 个 进 程，在 加 载 到 内 存 的 时 候，形 成 真 正 的 进 程 时 操 作 系 统 要 先 创 建 描 述 进 程 属 性 的 结 构 体 对 象，即 PCB，process ctrl block，被 称 做 进 程 控 制 块。PCB 是 进 程 属 性 的 集 合。
人 通 过 属 性 来 认 识 所 有 的 事 物 的，当 属 性 积 累 到 一 定 程 度，这 一 堆 属 性 的 集 合 就 是 目 标 描 述 的 事 物 对 象。